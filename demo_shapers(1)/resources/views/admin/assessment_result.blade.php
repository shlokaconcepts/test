                ...
                <div class="card-body">
                    <form method="get" class="js-datatable-filter-form row mb-3">
                        ...
                        <div class="col-md-3 mt-1 col-12">
                            <div class="form-group ">
                                <label for="form_category" class="form-label mb-0">Assessment Result:</label>
                                <select class="select2" name="result" id="result">
    ...
    <script>
        $(function() {
            ...
            $('#clear_filter').on('click', function() {
                ...
                @endif
                window.LaravelDataTables["assessmentdatatable-table"].draw();
            });
            ...
            $('#assessmentdatatable-table').on('draw.dt', function() {
            ...
            $('#assessmentdatatable-table').on('preXhr.dt', function(e, settings, data) {
            ...
            $('#submit_exam').submit(function(e) {
                e.preventDefault();
                var send_data = [];
                var date=$('#int_date').val();
                if(date==''){
                    Swal.fire({
                        position: 'center',
                        icon: 'warning',
                        text: "Interview date field is required",
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('admin.mark_absent_for_interview') }}",
                    data: {
                        data: send_data,
                    },
                    ...
                    success: function(response) {
                        if (response.status == true) {
                            Swal.fire({
                                ...
                                window.LaravelDataTables["assessmentdatatable-table"]
                                    .draw();
                            });
            ...
            $('.submit_btn').on('click', function(e) {
                e.preventDefault();
                var postData = new FormData(document.getElementById('editMark'));
                $.ajax({
                    type: 'POST',
                    data: postData,
                    url: "{{ route('admin.update-assessment-mark') }}",
                    contentType: false,
                    processData: false,
                    dataType: 'json',
                    success: function(response) {
                        if (response.status == true) {
                            Swal.fire({
                                ...
                                document.getElementById('editMark').reset();
                                window.LaravelDataTables["assessmentdatatable-table"]
                                    .draw();
        ...
        function EditMarkModal(selector, id) {
            $.ajax({
                url: "{{ url('admin/fetch-assessment-details') }}/" + id,
                type: "GET",
                dataType: 'json',
                success: function(res) {
                    if (res.status == true) {
                        ...
                        $('.aptitude_passing_mark').html(data.aptitude_passing_mark);
                        $('#technical').val(data.technical);
                        $('.technical_passing_mark').html(data.technical_passing_mark);
                        $('#behavior').val(data.behavior);
                        $('.behavior_passing_mark').html(data.behavior_passing_mark);
                        $('#results').val(data.result).change();
