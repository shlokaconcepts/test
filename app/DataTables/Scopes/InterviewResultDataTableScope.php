...
class InterviewResultDataTableScope implements DataTableScope
{
    /**
     * Apply a query scope.
     *
    ...
    public function apply($query)
    {
        ...
        if ($status = request('status')) {
            $query->where('candidate_interviews.status',$status);
        }
        ...
        $query->where('users.interview','1')->where('candidate_interviews.status','!=','Absent')->where('candidate_interviews.status','!=',null)->where('users.on_boarding','0');
        return $query;
    }
}
